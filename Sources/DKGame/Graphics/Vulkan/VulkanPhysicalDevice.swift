...
public class VulkanPhysicalDeviceDescription: CustomStringConvertible {
    ...
    public let queueFamily: [VkQueueFamilyProperties]
    public let extensions: [VulkanExtensionProperties]
    ...
    public init(device: VkPhysicalDevice) {
        ...
        self.queueFamily = .init(unsafeUninitializedCapacity: Int(queueFamilyCount)) {
    ...
    public var description: String {
        ...
        var desc = "VulkanPhysicalDevice(name: \"\(self.name)\", identifier:\(self.registryID), type:\(deviceType), API:\(apiVersion), QueueFamilies:\(self.queueFamily.count))"
        for (index, qf) in self.queueFamily.enumerated() {
        ...
        for (index, ext) in self.extensions.enumerated() {
            desc += "\n -- Device Extension[\(index)]: \"\(ext.name)\" (Version: \(ext.specVersion))"
        }
        return desc
    }
}
