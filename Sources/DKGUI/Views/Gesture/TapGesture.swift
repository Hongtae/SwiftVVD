...
public struct TapGesture : Gesture {
    ...
    public static func _makeGesture(gesture: _GraphValue<TapGesture>, inputs: _GestureInputs) -> _GestureOutputs<Void> {
        _GestureOutputs<Void>(recognizer: TapGestureRecognizer(gesture: gesture.value, inputs: inputs))
    }
...
class TapGestureRecognizer : _GestureRecognizer<TapGesture.Value> {
    ...
    var count = 0
    let maximumInterval: ContinuousClock.Duration = .seconds(0.5)
    let maximumDuration: ContinuousClock.Duration = .seconds(1.0)
